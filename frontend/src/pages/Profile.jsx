...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchMyOrders = async () => {
      try {
        ...
        console.error("Order profile synchronization error:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Modern touch design status badge style configurations
  const statusStyles = {
  ...
  // Derive global metrics parameters from live array data safely
  const completedOrdersCount = orders.filter(
    (o) => o.status?.toLowerCase() === "delivered",
  ).length;
  const totalInvestedAmount = orders.reduce(
    (acc, o) =>
      acc +
      ...
      <div className="max-w-5xl mx-auto space-y-8 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 backdrop-blur-md bg-zinc-900/40 border border-zinc-900 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 shadow-xl">
            <div className="flex items-center gap-4 sm:gap-5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-zinc-500 font-semibold tracking-wider font-mono">
                  {user?.email}
                </p>
                <span className="inline-flex bg-blue-500/10 text-blue-400 border border-blue-500/20 text-[10px] font-black px-2.5 py-0.5 rounded-md uppercase tracking-wider mt-1.5">
                  Account Type: {user?.role ? user.role.toUpperCase() : "USER"}
                </span>
              </div>
            </div>
          ...
          <div className="bg-zinc-900/60 border border-zinc-900 rounded-3xl p-6 flex justify-around lg:flex-col lg:justify-between lg:space-y-4 shadow-xl">
            <div>
              <span className="text-zinc-500 text-[10px] font-black uppercase tracking-widest block mb-0.5">
                Total Expenses
              </span>
        ...
        <div className="space-y-5">
          ...
          {loading ? (
            ...
            <div className="bg-zinc-900/20 border border-dashed border-zinc-900 rounded-3xl p-12 text-center max-w-md mx-auto shadow-xl">
              <div className="h-12 w-12 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center mx-auto mb-4 text-lg shadow-inner select-none">
                🧾
              </div>
              <p className="text-zinc-500 text-xs font-semibold px-4 leading-relaxed mb-4">
                You haven't placed any orders yet.
              </p>
              <Link
                to="/shop"
                className="inline-block bg-linear-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold px-5 py-2.5 rounded-xl transition-all shadow-md transform hover:-translate-y-0.5"
              >
                Start Shopping
              </Link>
            </div>
          ) : (
            /* ACTIVE DATA STREAM CARDS */
            <div className="grid grid-cols-1 gap-4 animate-fade-in">
              {orders.map((order) => {
                ...
                return (
                  <div
                    key={order._id}
                    className="backdrop-blur-md bg-zinc-900/30 border border-zinc-900/80 rounded-2xl p-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:border-zinc-700 transition-all duration-300 shadow-md group"
                  >
                    <div className="space-y-1.5 text-xs sm:text-sm text-zinc-400">
                      ...
                      <div className="flex gap-4 text-zinc-500 font-medium text-[10px] uppercase tracking-wider">
                        <p>
                          Placed On:{" "}
                          <span className="text-zinc-300 font-sans font-normal">
                        ...
                        <p>
                          Items:{" "}
                          <span className="text-zinc-300 font-sans font-normal">
                            {order.items?.length || 1} unit(s)
                          </span>
                        </p>
                      </div>
                      <p className="text-zinc-500 font-bold pt-1 flex items-baseline gap-1">
                        Total:
                        <strong className="text-emerald-400 text-base font-black tracking-tight transition-colors">
                          ₹
                    ...
                    <div className="sm:text-right flex sm:flex-col items-center sm:items-end justify-between border-t sm:border-t-0 border-zinc-900/50 pt-3 sm:pt-0">
                      <span
                        className={`inline-block border text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-wider ${statusStyles[normalizedStatus] || statusStyles.pending}`}
                      >
                        ● {order.status}
                      </span>
                    </div>
